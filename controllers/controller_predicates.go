...
func (SecretPredicates) Create(e event.CreateEvent) bool {
	...
	if value, ok := e.Meta.GetAnnotations()["fastly.amazee.io/watch"]; ok {
		if value == "true" {
			return true
		}
	}
...
func (SecretPredicates) Update(e event.UpdateEvent) bool {
	// handle "fastly.amazee.io/watch" annotation
	if value, ok := e.MetaNew.GetAnnotations()["fastly.amazee.io/watch"]; ok {
		if value == "true" {
			return true
		}
	}
	if value, ok := e.MetaOld.GetAnnotations()["fastly.amazee.io/watch"]; ok {
		if value == "true" {
			return true
		}
	}
...
func (IngressPredicates) Create(e event.CreateEvent) bool {
	// handle "fastly.amazee.io/watch" annotation
	if _, ok := e.Meta.GetAnnotations()["fastly.amazee.io/watch"]; ok {
		if value, ok2 := e.Meta.GetAnnotations()["fastly.amazee.io/service-id"]; ok2 {
			if value != "" {
				return true
			}
		}
...
func (IngressPredicates) Update(e event.UpdateEvent) bool {
	// handle "fastly.amazee.io/watch" annotation
	if watch, ok := e.MetaNew.GetAnnotations()["fastly.amazee.io/watch"]; ok {
		if _, okOld := e.MetaOld.GetAnnotations()["fastly.amazee.io/watch"]; okOld {
			if val, ok2 := e.MetaNew.GetAnnotations()["fastly.amazee.io/service-id"]; ok2 {
				if valOld2, okOld2 := e.MetaOld.GetAnnotations()["fastly.amazee.io/service-id"]; okOld2 {
					if val == valOld2 {
						return true
					}
				}
